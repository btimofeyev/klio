...
describe("planFamilyWeek availability integration", () => {
  it("keeps a learner-specific timed conflict isolated while avoiding the blocked interval", async () => {
    ...
    await planFamilyWeek({ supabase: admin, familyId, parentId: userId, anchorDate: "2026-07-20" });
    const assignments = await admin.from("assignments").select("student_id,scheduled_date,scheduled_time").eq("family_id", familyId).gte("scheduled_date", "2026-07-20").lte("scheduled_date", "2026-07-21");
  ...
  it("applies a family-wide conflict to both learners", async () => {
    ...
    if (conflict.error) throw conflict.error;
    await planFamilyWeek({ supabase: admin, familyId, parentId: userId, anchorDate: "2026-07-27" });
  ...
  it("fails safely without inserting a partial plan when no date is available", async () => {
    ...
    await expect(planFamilyWeek({ supabase: admin, familyId, parentId: userId, anchorDate: "2026-08-03" })).rejects.toMatchObject({ code: "FREQUENCY_OVER_CAPACITY" } satisfies Partial<FamilyWeekPlanError>);
  ...
  it("plans sibling parent time together while allowing safe independent parallel work", async () => {
    ...
    await planFamilyWeek({ supabase: admin, familyId, parentId: userId, anchorDate: "2026-08-10" });
