...
test("a parent checks source-backed Klio suggestions and corrections", async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.getByRole("heading", { name: "Check Klio" })).toBeVisible();
    await expect(page.getByText("Check what Klio understood before it uses it to help plan what comes next.")).toBeVisible();
    ...
    await expect(page.locator('.app-nav a[href="/app/activity"] b')).toHaveText("3");
    ...
    await expect(page.locator('.mobile-nav a[href="/app/activity"] b')).toHaveText("1");
